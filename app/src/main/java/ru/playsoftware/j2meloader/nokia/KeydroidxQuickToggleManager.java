 ...
 */
public class KeydroidxQuickToggleManager {

	private static final String TAG = "KeydroidxQuickToggle";
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					KeydroidxLog.w(TAG, "探测 mini_shizuku 状态失败，按离线处理", t);
					return;
				}
				Boolean old = sShizukuRunning;
				sShizukuRunning = now;
				KeydroidxLog.i(TAG, "mini_shizuku 状态: " + old + " -> " + now);
	...
	public static boolean isToggleOn(Context context, int type) {
		switch (type) {
			case KeydroidxQuickToggleItem.TYPE_WIFI:
				return isWifiOn(context);
			case KeydroidxQuickToggleItem.TYPE_DATA:
				return isMobileDataOn(context);
			case KeydroidxQuickToggleItem.TYPE_BLUETOOTH:
				return isBluetoothOn();
			case KeydroidxQuickToggleItem.TYPE_AIRPLANE:
				return isAirplaneModeOn(context);
			case KeydroidxQuickToggleItem.TYPE_TORCH:
				return isTorchOn();
			case KeydroidxQuickToggleItem.TYPE_SOUND:
				return isSoundOn(context);
			case KeydroidxQuickToggleItem.TYPE_ROTATE:
				return isRotateOn(context);
			case KeydroidxQuickToggleItem.TYPE_LOCK:
				return false; // 一键锁屏为瞬态操作
			case KeydroidxQuickToggleItem.TYPE_BRIGHTNESS:
				return isBrightnessHigh(context);
			case KeydroidxQuickToggleItem.TYPE_LOCATION:
				return isLocationOn(context);
			case KeydroidxQuickToggleItem.TYPE_HOTSPOT:
				return isHotspotOn(context);
			case KeydroidxQuickToggleItem.TYPE_SAVER:
				return isSaverOn(context);
			case KeydroidxQuickToggleItem.TYPE_FREEZE:
			case KeydroidxQuickToggleItem.TYPE_UNFREEZE:
			case KeydroidxQuickToggleItem.TYPE_CLEAN_BG:
				return false; // 一键冻结 / 一键解冻 / 清理后台为瞬态动作
			case KeydroidxQuickToggleItem.TYPE_SHUTDOWN:
			case KeydroidxQuickToggleItem.TYPE_REBOOT:
			case KeydroidxQuickToggleItem.TYPE_RECOVERY:
			case KeydroidxQuickToggleItem.TYPE_FASTBOOT:
	...
	public static void toggleWifi(final Context context, final boolean targetOn) {
		if (Build.VERSION.SDK_INT < 29) {
			try {
				...
				KeydroidxLog.w(TAG, "wifi setWifiEnabled failed: " + e.getMessage());
			}
		}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				if (Shizuku.isRunning()) {
					...
					KeydroidxLog.i(TAG, "Shizuku wifi toggle: " + res);
					if (res) return;
				}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				if (Shizuku.isRunning()) {
					...
					boolean res = Shizuku.exec(cmd);
					KeydroidxLog.i(TAG, "Shizuku data toggle: " + res);
					return;
				}
				...
				try {
					...
					KeydroidxLog.w(TAG, "setMobileDataEnabled failed: " + e.getMessage());
				}
	...
	public static void toggleBluetooth(final Context context, final boolean targetOn) {
		if (Build.VERSION.SDK_INT < 33) {
			try {
				...
				KeydroidxLog.w(TAG, "bluetooth enable/disable failed: " + e.getMessage());
			}
		}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				if (Shizuku.isRunning()) {
					...
					boolean res = Shizuku.exec(cmd);
					KeydroidxLog.i(TAG, "Shizuku bluetooth toggle: " + res);
					if (res) return;
				}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				if (Shizuku.isRunning()) {
					...
					boolean res = Shizuku.exec(cmd);
					KeydroidxLog.i(TAG, "Shizuku airplane toggle: " + res);
					if (res) return;
				}
			...
			public void run() {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
					try {
						...
						KeydroidxLog.w(TAG, "CameraManager setTorchMode error: " + t.getMessage());
					}
				}
				...
				try {
					...
					KeydroidxLog.w(TAG, "Legacy camera torch error: " + t.getMessage());
					if (legacyCamera != null) {
	...
	public static int currentBrightnessLevel(Context context) {
		...
		KeydroidxLog.i(TAG, "亮度初始档位: " + brightnessLevelName(level));
		return level;
	}
	...
	public static String getBrightnessIconUnicode(Context context) {
		switch (currentBrightnessLevel(context)) {
			case LEVEL_AUTO: return KeydroidxIcons.TOGGLE_BRIGHTNESS_AUTO;
			case LEVEL_HIGH: return KeydroidxIcons.TOGGLE_BRIGHTNESS_HIGH;
			case LEVEL_MEDIUM: return KeydroidxIcons.TOGGLE_BRIGHTNESS_MEDIUM;
			case LEVEL_LOW:
			default: return KeydroidxIcons.TOGGLE_BRIGHTNESS_LOW;
		}
	}
	...
	public static void toggleBrightness(final Context context) {
		...
		if (!isShizukuRunningCached()) {
			KeydroidxLog.w(TAG, "mini_shizuku 未激活(缓存)，亮度改为跳转系统设置页");
		...
		KeydroidxLog.i(TAG, "亮度档位: " + brightnessLevelName(cur)
	...
	private static void dispatchBrightnessCommand(final Context context, final int target) {
		...
		KeydroidxLog.i(TAG, "Shizuku 亮度写入: " + sent);
		if (sent) {
			return;
		}
		...
		KeydroidxLog.w(TAG, "亮度写入失败，跳转系统设置页");
	...
	public static void toggleFreezeAll(final Context context) {
		...
		KeydroidxFreezeManager.getInstance(appCtx).freezeAll(null);
	}

	public static void toggleUnfreezeAll(final Context context) {
		if (context == null) return;
		final Context appCtx = context.getApplicationContext();
		KeydroidxFreezeManager.getInstance(appCtx).unfreezeAll(null);
	}
	...
	public static void toggleCleanBg(final Context context) {
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				KeydroidxSettingsStorage storage = new KeydroidxSettingsStorage(appCtx);
				java.util.Set<String> protectedSet = storage.getProtectedPackages();
				final int clearedCount = KeydroidxBgManagerHelper.clearBackgroundTasks(appCtx, protectedSet);
	...
	// 桌面触发前会二次确认（见 KeydroidxQuickToggleItem#isPowerAction）。
	...
	private static void execPowerCommand(final Context context, final String command, final String label) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				final boolean sent;
				if (!Shizuku.isRunning()) {
					KeydroidxLog.w(TAG, label + "失败：mini_shizuku 服务未运行");
					sent = false;
				} else {
					sent = Shizuku.exec(command);
					KeydroidxLog.i(TAG, label + "命令已发送: " + command + " -> " + sent);
				}
				if (!sent) {
