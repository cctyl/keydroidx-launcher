 ...
 */
public class KeydroidxMidletControlReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (MidletThread.hasInstance()) {
			KeydroidxLog.i("MidletControl", "收到清除广播：销毁挂机 MIDlet");
			MidletThread.destroyApp();
		} else {
			// 进程内已无 MIDlet（竞态残留）：直接自杀清场（通知/Service 随进程消亡）
			KeydroidxLog.i("MidletControl", "收到清除广播：无实例，直接结束 :midlet 进程");
