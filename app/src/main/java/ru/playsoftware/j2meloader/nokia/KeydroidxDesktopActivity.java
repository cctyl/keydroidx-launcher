 ...
 * 此时应清除返回栈并回到桌面待机屏。
 */
public class KeydroidxDesktopActivity extends KeydroidxBaseActivity
		implements KeydroidxPageHost, KeyResolver {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		KeydroidxWallpaper.preloadAsync(this, null);
		applyCurrentTheme();
		...
		if (existing == null) {
			if (!keyBinding.isWizardDone()) {
				KeydroidxLog.i("Desktop", "首次启动：进入按键绑定向导");
				loadWizardFragment();
			} else {
				loadDesktopFragment();
			}
	...
	private void checkCorePermissionsOnStartup() {
		...
		getWindow().getDecorView().post(() -> {
			...
			if (KeydroidxPermissionManager.isCorePermissionsGranted(this)) {
				KeydroidxLog.i("Desktop", "启动时核心权限自检：全集就绪");
				return;
			}
			KeydroidxLog.i("Desktop", "启动时核心权限自检：存在缺失项，启动批量引导");
			KeydroidxPermissionManager.requestCorePermissions(
					this,
					...
					new OnPermissionCallback() {
						...
						public void onGranted(java.util.List<String> permissions, boolean allGranted) {
							KeydroidxLog.i("Desktop", "启动时核心权限自检：已补齐 " + permissions);
						}

						@Override
						public void onDenied(java.util.List<String> permissions, boolean doNotAskAgain) {
							KeydroidxLog.w("Desktop", "启动时核心权限自检：用户拒绝 " + permissions + ", doNotAskAgain=" + doNotAskAgain);
						}
					});
		});
	}
	...
	public void reloadKeyBindings() {
		if (keyBinding != null) {
			keyBinding.reload();
			KeydroidxLog.i("Desktop", "reloadKeyBindings 完成");
		}
	}
	...
	public boolean isDefaultLauncher() {
		return KeydroidxLauncherUtils.isDefaultLauncher(this);
	}
	...
	public void requestSetDefaultLauncher() {
		KeydroidxLog.i("Desktop", "requestSetDefaultLauncher 调用，api=" + Build.VERSION.SDK_INT);
		try {
			...
			KeydroidxLog.e("Desktop", "requestSetDefaultLauncher 失败", e);
		}
	}
	...
	public void finish() {
		...
		if (fm.getBackStackEntryCount() > 0) {
			KeydroidxLog.i("Desktop", "finish() 被子页面调用 -> 出栈返回上一层");
			exitCurrent();
			return;
		}
		super.finish();
	 ...
	 * 动作取值与 {@link KeydroidxKeyBinding#ACTION_UP} 等常量一致（也即 common KeydroidxKeyAction 的取值）。
	 */
	@Override
	public int resolveAction(@NonNull KeyEvent event) {
		// 纯查询，不做 reload：dispatchKeyEvent 已在每次按键前 reload，避免按键热路径重复加载
		return keyBinding == null ? KeydroidxKeyAction.UNKNOWN : keyBinding.resolveAction(event);
	}

	/**
	 * 重新读取当前页面的 {@link KeydroidxPage} 声明并装配底部菜单栏。
	 * <p>
	...
	public void refreshPageBar() {
		...
		// 用 common 的 KeydroidxPage 判定：桌面页面（KeydroidxPage 继承它）与 common 自带页面
		// （如意见反馈页）都能被装配底栏。
		if (f instanceof io.github.cctyl.nokia.common.ui.page.KeydroidxPage) {
			io.github.cctyl.nokia.common.ui.page.KeydroidxPage page =
					(io.github.cctyl.nokia.common.ui.page.KeydroidxPage) f;
			...
			KeydroidxLog.d("Desktop", "refreshPageBar 装配 " + f.getClass().getSimpleName()
					+ " left=" + left + " center=" + center + " right=" + right);
			setBottomBar(left == null ? null : left.toString(),
					center == null ? null : center.toString(),
					right == null ? null : right.toString());
		} else {
			KeydroidxLog.d("Desktop", "refreshPageBar: 当前 Fragment 未实现 KeydroidxPage（"
	...
	protected void onResume() {
		...
		KeydroidxLog.i("Desktop", "onResume 已调度 StatusBarController 延迟启动");
	}
	...
	private void scheduleStatusBarStart() {
		...
		final Runnable task = new Runnable() {
			...
			public void run() {
				if (!resumedFlag) {
					KeydroidxLog.d("Desktop", "状态栏延迟启动取消：Activity 已非 resumed");
					return;
				}
				...
				try {
					...
					KeydroidxLog.w("Desktop", "状态栏延迟启动异常: " + e.getMessage());
				}
				long elapsed = System.currentTimeMillis() - start;
				KeydroidxLog.i("Desktop", "StatusBarController.start 完成，耗时 " + elapsed + "ms");
			}
		};
	...
	public void onWindowFocusChanged(boolean hasFocus) {
		...
		if (hasFocus) {
			...
			KeydroidxLog.i("Desktop", "onWindowFocusChanged hasFocus=true, 当前方向="
	...
	public void applyCurrentTheme() {
		KeydroidxSettingsStorage storage = new KeydroidxSettingsStorage(this);
		KeydroidxTheme.ThemeDef theme = storage.getTheme();
		applyWallpaper(theme);
		View bottomBar = findViewById(R.id.bottomPanel);
		if (bottomBar != null) {
			bottomBar.setBackground(KeydroidxTheme.createSoftKeyDrawable(theme));
		}
	 ...
	 */
	private void applyWallpaper(final KeydroidxTheme.ThemeDef theme) {
		final View wall = findViewById(R.id.wallpaper);
		if (wall == null) {
			return;
		}
		wall.setBackground(KeydroidxTheme.createBackgroundDrawable(theme));
		// 未设置自定义壁纸：主题渐变就是最终背景
		if (!KeydroidxWallpaper.hasCustomWallpaper(this)) {
			return;
		}
		// 已解码：一次性铺好「渐变 + 壁纸」
		if (KeydroidxWallpaper.isBitmapReady()) {
			wall.setBackground(KeydroidxWallpaper.createWallpaperDrawable(this, theme));
			return;
		}
		KeydroidxWallpaper.preloadAsync(this, new Runnable() {
			@Override
			public void run() {
				if (isFinishing()) {
					return;
				...
				if (w != null) {
					w.setBackground(KeydroidxWallpaper.createWallpaperDrawable(KeydroidxDesktopActivity.this, theme));
				}
			}
		});
	}
	...
	protected void onNewIntent(Intent intent) {
		...
		if (isHomeIntent(intent)) {
			KeydroidxLog.i("Desktop", "收到 HOME intent，回到桌面待机");
			goHome();
		} else {
			KeydroidxLog.d("Desktop", "onNewIntent 非 HOME intent，忽略");
		}
	}
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
		if (event.getAction() != KeyEvent.ACTION_DOWN) {
			...
			if (event.getAction() == KeyEvent.ACTION_UP
				...
				KeydroidxLog.i("Desktop", "锁屏按键抬起（UP）：执行锁屏");
				lockScreen();
				return true;
			}
			...
			if (event.getKeyCode() == lastHandledDownKeyCode) {
				KeydroidxLog.d("Desktop", "dispatchKeyEvent 拦截已消费按键的 "
						+ (event.getAction() == KeyEvent.ACTION_UP ? "UP" : "REPEAT")
						+ " " + KeydroidxKeyBinding.keyName(event.getKeyCode()));
				return true;
			}
			return super.dispatchKeyEvent(event);
		}

		KeydroidxLog.d("Desktop", "dispatchKeyEvent 收到按下 " + KeydroidxKeyBinding.keyName(event.getKeyCode()));
		...
		boolean recording = curForRec instanceof KeydroidxKeyRecorder
				&& ((KeydroidxKeyRecorder) curForRec).isRecording();
		...
		if (event.getRepeatCount() > 0) {
			...
			if (recording || !isDirectionAction(repeatAction)) {
				KeydroidxLog.d("Desktop", "吞掉长按连发 " + KeydroidxKeyBinding.keyName(event.getKeyCode())
						+ " repeat=" + event.getRepeatCount()
						+ " action=" + KeydroidxKeyBinding.getActionName(repeatAction));
				return true;
			}
			KeydroidxLog.d("Desktop", "方向键连发放行 repeat=" + event.getRepeatCount());
		}

		if (recording) {
			KeydroidxKeyRecorder rec = (KeydroidxKeyRecorder) curForRec;
			int kc = event.getKeyCode();
			// 录制态下：用户按下的任意物理键（含返回键）都照常录成当前动作的绑定，
			// 不做任何忽略。\"跳过\"只通过屏幕上的触摸按钮触发（onSkipCurrent），
			// 不会在这里用返回键实现。
			KeydroidxLog.i("Desktop", "录制态捕获按键 " + KeydroidxKeyBinding.keyName(kc));
		...
		if (action < 0) {
			...
			if (event.getKeyCode() == KeyEvent.KEYCODE_BACK) {
				...
				if (backHost instanceof KeydroidxFocusHost
					...
					KeydroidxLog.d("Desktop", "未绑定返回键 -> host.onBack()");
					((KeydroidxFocusHost) backHost).onBack();
					lastHandledDownKeyCode = event.getKeyCode();
					return true;
				}
			}
			// 未绑定的按键：允许系统继续处理（如音量键仍然调整音量）
			KeydroidxLog.d("Desktop", "未绑定的按键 " + KeydroidxKeyBinding.keyName(event.getKeyCode())
					+ "，交给系统处理");
			resetLastHandledKeyCode();
			return super.dispatchKeyEvent(event);
		}
		...
		if (dispatchActionToHost(action)) {
			KeydroidxLog.d("Desktop", "动作 " + KeydroidxKeyBinding.getActionName(action)
					+ " 已被当前 Fragment 消费");
			lastHandledDownKeyCode = event.getKeyCode();
			return true;
		}

		KeydroidxLog.d("Desktop", "dispatchKeyEvent 未消费 " + KeydroidxKeyBinding.keyName(event.getKeyCode())
				+ "，交给系统");
		resetLastHandledKeyCode();
		return super.dispatchKeyEvent(event);
	}
	...
	private static boolean isDirectionAction(int action) {
		return action == KeydroidxKeyBinding.ACTION_UP
				|| action == KeydroidxKeyBinding.ACTION_DOWN
				|| action == KeydroidxKeyBinding.ACTION_LEFT
				|| action == KeydroidxKeyBinding.ACTION_RIGHT;
	}

	/**
	 * 把动作分发给当前中间面板 Fragment（KeydroidxFocusHost）。
	...
	private boolean dispatchActionToHost(int action) {
		...
		KeydroidxFocusHost host = (KeydroidxFocusHost) current;
		boolean handled;
		switch (action) {
	...
	private void bindBottomBarTouch() {
		...
		if (left != null) {
			left.setOnClickListener(v -> {
				KeydroidxLog.i("Desktop", "触摸点击 -> 左软键");
				flashBottomBar(KeydroidxKeyBinding.ACTION_SOFT_LEFT);
				dispatchActionToHost(KeydroidxKeyBinding.ACTION_SOFT_LEFT);
			});
		}
		if (center != null) {
			center.setOnClickListener(v -> {
				KeydroidxLog.i("Desktop", "触摸点击 -> 确认键");
				flashBottomBar(KeydroidxKeyBinding.ACTION_SELECT);
				dispatchActionToHost(KeydroidxKeyBinding.ACTION_SELECT);
			});
		}
		if (right != null) {
			right.setOnClickListener(v -> {
				KeydroidxLog.i("Desktop", "触摸点击 -> 右软键");
				flashBottomBar(KeydroidxKeyBinding.ACTION_SOFT_RIGHT);
				dispatchActionToHost(KeydroidxKeyBinding.ACTION_SOFT_RIGHT);
			});
		}
	}

	/** 一键锁屏（委托给 KeydroidxLockScreen 工具类）。 */
	private void lockScreen() {
		KeydroidxLockScreen.lock(this);
	}

	private void loadWizardFragment() {
		KeydroidxLog.i("Desktop", "加载首次启动按键绑定向导 Fragment");
		getSupportFragmentManager().beginTransaction()
				.replace(R.id.midPanel, new KeydroidxKeyBindWizardFragment())
				.commit();
		postReportPageState();
	}
	...
	public void openMenu() {
		KeydroidxLog.i("Desktop", "导航 -> 功能表");
		switchFragment(new KeydroidxMenuFragment());
	}

	public void openBox() {
		KeydroidxLog.i("Desktop", "导航 -> 应用程序");
		switchFragment(new KeydroidxBoxFragment());
	}

	/** 打开桌面设置界面 */
	public void openDesktopSettings() {
		KeydroidxLog.i("Desktop", "导航 -> 桌面设置");
		switchFragment(new KeydroidxDesktopSettingsFragment());
	}

	/** 打开通知中心列表页（功能表格子 / 桌面通知条共用入口）。 */
	public void openNotificationCenter() {
		KeydroidxLog.i("Desktop", "导航 -> 通知中心");
		switchFragment(new KeydroidxNotificationCenterFragment());
	}
	...
	public void exitCurrent() {
		FragmentManager fm = getSupportFragmentManager();
		if (fm.getBackStackEntryCount() > 0) {
			KeydroidxLog.i("Desktop", "exitCurrent 出栈返回上一层");
			fm.popBackStack();
		} else {
			KeydroidxLog.i("Desktop", "exitCurrent 无返回栈，finish()");
			finish();
		}
	}
	...
	/**
	 * 检测当前 Fragment 是否为桌面主界面（KeydroidxDesktopFragment），通过 TCP 上报给
	...
	private void reportPageState(boolean force) {
		Fragment f = getSupportFragmentManager().findFragmentById(R.id.midPanel);
		String state = (f instanceof KeydroidxDesktopFragment) ? "main" : "sub";
		if (!force && state.equals(lastReportedPageState)) return;
		lastReportedPageState = state;
		KeydroidxLog.i("Desktop", "上报页面状态: " + state
				+ " (fragment=" + (f != null ? f.getClass().getSimpleName() : "null") + ")");
		new Thread(() -> {
			try {
				Shizuku.setPageState("main".equals(state));
			} catch (Exception e) {
				KeydroidxLog.w("Desktop", "上报页面状态失败: " + e.getMessage());
			}
		}, "nokia-page-state").start();
	}

	public static KeydroidxDesktopActivity getInstance() {
		return sInstance;
	}
	...
	private void goHome() {
		...
		if (!(cur instanceof KeydroidxDesktopFragment)) {
			fm.beginTransaction()
					.replace(R.id.midPanel, new KeydroidxDesktopFragment())
	...
	private void loadDesktopFragment() {
		KeydroidxLog.i("Desktop", "加载初始桌面 Fragment");
		getSupportFragmentManager().beginTransaction()
				.replace(R.id.midPanel, new KeydroidxDesktopFragment())
				.commit();
		postReportPageState();
