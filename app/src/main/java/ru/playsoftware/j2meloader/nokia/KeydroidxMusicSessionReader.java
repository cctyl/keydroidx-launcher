 ...
 * <p>
 * <b>前提：</b>需要用户授予「通知使用权」（见 {@link KeydroidxNotificationListenerService}）。
 ...
 */
public final class KeydroidxMusicSessionReader {

	private static final String TAG = "MusicSession";

	private KeydroidxMusicSessionReader() {}
	...
	private static MusicState readInternal(Context ctx, String targetPackage) {
		...
		try {
			...
			KeydroidxLog.w(TAG, "获取 MediaSessionManager 失败: " + e.getMessage());
			return null;
		}
		...
		try {
			controllers = msm.getActiveSessions(
					new ComponentName(ctx, KeydroidxNotificationListenerService.class));
		} catch (SecurityException e) {
			// 通知使用权已被撤销（缓存过期），本次按不可用处理
			KeydroidxLog.w(TAG, "通知使用权未授予，MediaSession 读取被拒");
			return null;
		} catch (Exception e) {
			KeydroidxLog.w(TAG, "getActiveSessions 失败: " + e.getMessage());
			return null;
		}
		...
		if (target == null) {
			KeydroidxLog.d(TAG, "目标包无活跃 MediaSession: " + targetPackage);
			return null;
		}
		...
		if (!st.hasData()) {
			KeydroidxLog.d(TAG, "MediaSession 无标题数据: " + targetPackage);
			return null;
		}
		KeydroidxLog.i(TAG, "MediaSession 读到播放状态: " + st.title
	...
	public static boolean isNotificationListenerEnabled(Context ctx) {
		...
		try {
			...
			KeydroidxLog.w(TAG, "读取 enabled_notification_listeners 失败: " + e.getMessage());
			return false;
		}
	...
	public static void showGrantPrompt(FragmentManager fm, final Context ctx, boolean neverAsk) {
		...
		items.add(new KeydroidxOptionsDialog.OptionItem(KeydroidxIcons.ICON_INFO,
				"用于通知中心与音乐播放状态", false, false, null));
		items.add(new KeydroidxOptionsDialog.OptionItem(KeydroidxIcons.ICON_SETTINGS, "去开启", true, false,
				new Runnable() {
					@Override
					public void run() {
		...
		items.add(new KeydroidxOptionsDialog.OptionItem(KeydroidxIcons.ICON_CLOSE, "以后再说", true, false, null));
		if (neverAsk) {
			items.add(new KeydroidxOptionsDialog.OptionItem(KeydroidxIcons.ICON_CLOSE, "不再提示", true, false,
					new Runnable() {
						@Override
						public void run() {
							KeydroidxSettingsStorage.setNotifyAccessPromptDisabled(ctx, true);
						}
					}));
		}
		KeydroidxOptionsDialog.show(fm, "需要通知使用权", items);
		KeydroidxLog.i(TAG, "已弹出通知使用权授予提示");
	}
	...
	public static boolean openNotificationListenerSettings(Context ctx) {
		...
		try {
			...
			KeydroidxLog.e(TAG, "打开通知使用权设置失败", e);
			return false;
		}
	}
}
