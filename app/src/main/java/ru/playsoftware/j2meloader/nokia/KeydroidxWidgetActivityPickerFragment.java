 ...
 */
public class KeydroidxWidgetActivityPickerFragment extends KeydroidxPageFragment {
	...
	public static KeydroidxWidgetActivityPickerFragment newAddMode(String packageName, String appLabel) {
		KeydroidxWidgetActivityPickerFragment f = new KeydroidxWidgetActivityPickerFragment();
		Bundle b = new Bundle();
	...
	public static KeydroidxWidgetActivityPickerFragment newEditMode(String packageName, String appLabel, int editIndex) {
		KeydroidxWidgetActivityPickerFragment f = new KeydroidxWidgetActivityPickerFragment();
		Bundle b = new Bundle();
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		KeydroidxLog.i(TAG, "初始化 mode=" + mode + " pkg=" + packageName
		...
		view.post(() -> {
			...
			buildPage();
			setInitialFocus();
			KeydroidxLog.i(TAG, "Activity选择页初始化完成：共 " + activities.size()
	...
	private void computeRowsPerPage() {
		...
		KeydroidxLog.i(TAG, "computeRowsPerPage: rowsPerPage=" + rowsPerPage
	...
	private void loadActivities() {
		if (packageName == null || packageName.isEmpty()) {
			KeydroidxLog.w(TAG, "packageName 为空，无法加载 Activity");
			return;
		}
		...
		try {
			...
			if (pkgInfo.activities == null || pkgInfo.activities.length == 0) {
				KeydroidxLog.i(TAG, "应用 " + packageName + " 没有任何 Activity");
				return;
			}
			...
			for (ActivityInfo ai : pkgInfo.activities) {
				...
				if (seenClassNames.contains(ai.name)) {
					KeydroidxLog.d(TAG, "去重跳过: " + ai.name);
					continue;
				}
				...
				// 过滤条件
				if (!ai.exported) {
					KeydroidxLog.d(TAG, "排除非exported Activity: " + ai.name);
					continue;
				}
				if (launcherNames.contains(ai.name)) {
					KeydroidxLog.d(TAG, "排除Launcher Activity: " + ai.name);
					continue;
				}
				...
				if (labelCs != null && labelCs.length() > 0) {
					...
					if (appLabelStr != null && appLabelStr.equals(rawLabel)) {
						...
						KeydroidxLog.d(TAG, "Activity label 回退到应用名 \"" + rawLabel
								+ "\"，改用类名简称: " + label);
					} else {
						label = rawLabel;
					}
			...
			KeydroidxLog.i(TAG, "加载 Activity: " + activities.size() + " 个（已过滤exported/launcher/重复）");
		} catch (PackageManager.NameNotFoundException e) {
			KeydroidxLog.e(TAG, "应用未找到: " + packageName, e);
		}
	}
	...
	private void buildPage() {
		...
		if (activities.isEmpty()) {
			...
			KeydroidxLog.i(TAG, "无可用 Activity，显示空状态");
			return;
		}
		...
		for (int i = 0; i < count; i++) {
			...
			row.setLayoutParams(new LinearLayout.LayoutParams(
					...
					KeydroidxDimens.dp(getResources(), ROW_H_DP)));
			...
			// 间距
			row.addView(spaceView(KeydroidxDimens.dp(getResources(), 5), 1));
			...
			KeydroidxFontManager.textSize(tv, 11);
	...
	private void pageNext() {
		...
		if (pageIndex < totalPages - 1) {
			...
			focusIndex = 0;
			applyListHighlight();
			KeydroidxLog.d(TAG, "翻页(下) -> " + (pageIndex + 1) + "/" + totalPages);
		}
	}
	...
	private void pagePrev() {
		...
		if (pageIndex > 0) {
			...
			applyListHighlight();
			KeydroidxLog.d(TAG, "翻页(上) -> " + (pageIndex + 1) + "/" + totalPages);
		}
	}
	...
	public boolean onSelect() {
		...
		KeydroidxLog.i(TAG, "选中 Activity label=" + entry.label + " className=" + entry.className);

		// 进入步骤3：名称输入
		KeydroidxWidgetActivityNameFragment nameFrag;
		if (MODE_EDIT.equals(mode)) {
			nameFrag = KeydroidxWidgetActivityNameFragment.newEditMode(
					packageName, entry.className, entry.label, editIndex);
		} else {
			nameFrag = KeydroidxWidgetActivityNameFragment.newAddMode(
					packageName, entry.className, entry.label);
		}
		((KeydroidxDesktopActivity) requireActivity()).openFragment(nameFrag);
		return true;
	}
	...
	public boolean onSoftRight() {
		KeydroidxLog.i(TAG, "右软键：返回步骤1（选择应用）");
