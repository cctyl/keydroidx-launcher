 ...
 * 按下任意物理键即完成绑定。
 */
public class KeydroidxKeyBindFragment extends KeydroidxPageFragment implements KeydroidxKeyRecorder {

	private KeydroidxKeyBinding keyBinding;
	private View[] itemViews = new View[KeydroidxKeyBinding.ACTION_COUNT];
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		recordStatusBar.setOnClickListener(v -> {
			KeydroidxLog.i("KeyBind", "触摸点击录制状态栏 -> 取消录制");
			onSkipCurrent();
		});
	...
	private void buildList() {
		...
		for (int i = 0; i < KeydroidxKeyBinding.ACTION_COUNT; i++) {
			...
			LinearLayout.LayoutParams lpDiv = new LinearLayout.LayoutParams(
					...
					KeydroidxDimens.dp(getResources(), 1));
			lpDiv.leftMargin = KeydroidxDimens.dp(getResources(), 12);
			lpDiv.rightMargin = KeydroidxDimens.dp(getResources(), 12);
	...
	private View createRow(int action) {
		...
		row.setPadding(KeydroidxDimens.dp(getResources(), 12), KeydroidxDimens.dp(getResources(), 8),
				KeydroidxDimens.dp(getResources(), 12), KeydroidxDimens.dp(getResources(), 8));

		// 左侧：动作名
		TextView tvAction = new TextView(requireContext());
		tvAction.setText(KeydroidxKeyBinding.getActionName(action));
		tvAction.setTextColor(0xFFFFFFFF);
		KeydroidxFontManager.textSize(tvAction, 11);
		...
		if (KeydroidxKeyBinding.isBound(kc)) {
		...
		KeydroidxFontManager.textSize(tvHint, 11);
		row.addView(tvHint);
	...
	void startRecording(int action) {
		...
		recordStatusText.setText("正在录制: " + KeydroidxKeyBinding.getActionName(action) + " — 请按目标键（点此处取消）");
		KeydroidxLog.i("KeyBind", "开始录制 action=" + KeydroidxKeyBinding.getActionName(action)
				+ "，等待物理按键...");
	}
	...
	public void onKeyRecorded(int keycode) {
		...
		KeydroidxLog.i("KeyBind", "录制完成 action=" + KeydroidxKeyBinding.getActionName(action)
				+ " 捕获 " + KeydroidxKeyBinding.keyName(keycode));

		int occupied = keyBinding.getActionForKeyCode(keycode);
		if (occupied >= 0 && occupied != action) {
			// 该键已被其它动作占用，进入确认模式（复用方向键/确认/返回导航）
			KeydroidxLog.w("KeyBind", "录制冲突：" + KeydroidxKeyBinding.keyName(keycode)
					+ " 已被 " + KeydroidxKeyBinding.getActionName(occupied) + " 占用");
	...
	public void onSkipCurrent() {
		...
		recordingAction = -1;
		recordStatusBar.setVisibility(View.GONE);
		KeydroidxLog.i("KeyBind", "跳过录制 action=" + KeydroidxKeyBinding.getActionName(action)
				+ "（保留默认 " + KeydroidxKeyBinding.keyName(keyBinding.getKeyCode(action)) + "）");
	}
	...
	private void enterConfirm(int action, int occupied, int keycode) {
		...
		// 底部菜单栏由 KeydroidxPage 声明动态装配（冲突模式软键不参与，全凭触摸）
		...
		updateConfirmText();
		KeydroidxLog.i("KeyBind", "进入冲突确认模式，触摸点击取消/覆盖按钮选择");
	}
	...
	private void buildConfirmButtons() {
		...
		int padH = KeydroidxDimens.dp(getResources(), 8);
		int padV = KeydroidxDimens.dp(getResources(), 4);
	...
	private TextView buildConfirmButton(String text, int color, View.OnClickListener listener) {
		...
		KeydroidxFontManager.textSize(btn, 11);
		btn.setClickable(true);
		btn.setFocusable(true);
		btn.setBackground(KeydroidxTheme.createSelectionDrawable(requireContext(), 4));
		btn.setOnClickListener(listener);
		LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
				LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
		lp.setMarginStart(KeydroidxDimens.dp(getResources(), 4));
	...
	private void doConfirm() {
		if (confirmChoice == 1) {
			KeydroidxLog.i("KeyBind", "用户选择覆盖：解除 "
					+ KeydroidxKeyBinding.getActionName(confirmOccupied)
					+ "，绑定到 " + KeydroidxKeyBinding.getActionName(confirmAction));
			applyBinding(confirmAction, confirmKeycode);
		} else {
			KeydroidxLog.i("KeyBind", "用户取消覆盖，保持 "
					+ KeydroidxKeyBinding.getActionName(confirmOccupied) + " 不变");
			Toast.makeText(requireContext(), "已取消，绑定未更改", Toast.LENGTH_SHORT).show();
		}
		confirming = false;
		confirmAction = confirmKeycode = confirmOccupied = -1;

		// 底部菜单栏由 KeydroidxPage 声明动态装配（恢复列表页左右软键文案）
		KeydroidxDesktopActivity host = (KeydroidxDesktopActivity) requireActivity();
		host.refreshPageBar();
	...
	private void restoreRecordStatusBar() {
		...
		dot.setLayoutParams(new LinearLayout.LayoutParams(
				KeydroidxDimens.dp(getResources(), 8), KeydroidxDimens.dp(getResources(), 8)));
		LinearLayout.LayoutParams dotLp = (LinearLayout.LayoutParams) dot.getLayoutParams();
		dotLp.setMargins(0, 0, KeydroidxDimens.dp(getResources(), 6), 0);
		dot.setBackground(KeydroidxTheme.createSelectionDrawable(requireContext(), 4));
		...
		KeydroidxFontManager.textSize(recordStatusText, 11);
	...
	public boolean onDirection(int direction) {
		...
		KeydroidxLog.d("KeyBind", "onDirection " + KeydroidxKeyBinding.getActionName(direction)
				+ " focus=" + focusIndex);
		if (focusIndex < 0) {
			setFocusIndex(0);
			return true;
		...
		switch (direction) {
			case KeydroidxKeyBinding.ACTION_UP:
				setFocusIndex(focusIndex > 0 ? focusIndex - 1 : KeydroidxKeyBinding.ACTION_COUNT - 1);
				return true;
			case KeydroidxKeyBinding.ACTION_DOWN:
				setFocusIndex(focusIndex < KeydroidxKeyBinding.ACTION_COUNT - 1 ? focusIndex + 1 : 0);
				return true;
			default:
	...
	public boolean onSelect() {
		...
		if (SystemClock.uptimeMillis() - enteredAt < ENTRY_DEBOUNCE_MS) {
			KeydroidxLog.i("KeyBind", "进入防抖窗口（"
					+ (SystemClock.uptimeMillis() - enteredAt) + "ms），忽略 SELECT，不触发录制");
			return true;
		}
		KeydroidxLog.d("KeyBind", "onSelect focus=" + focusIndex);
		if (focusIndex >= 0 && focusIndex < KeydroidxKeyBinding.ACTION_COUNT) {
	...
	public boolean onSoftLeft() {
		...
		KeydroidxLog.d("KeyBind", "onSoftLeft -> 等同选择");
	...
	public boolean onSoftRight() {
		...
		KeydroidxLog.d("KeyBind", "onSoftRight -> 返回");
		// 右软键 = 返回
	...
	public boolean onBack() {
		...
		KeydroidxLog.d("KeyBind", "onBack -> 返回");
		((KeydroidxDesktopActivity) requireActivity()).exitCurrent();
		return true;
	}
