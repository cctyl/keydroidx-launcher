 ...
 */
public class KeydroidxKeyProvider extends ContentProvider {
	...
	public boolean onCreate() {
		KeydroidxLog.i("KeyProvider", "KeydroidxKeyProvider 初始化");
		return true;
	}
	...
	public Cursor query(@NonNull Uri uri, @Nullable String[] projection,
		...
		KeydroidxLog.d("KeyProvider", "收到查询请求: uri=" + uri);

		if (getContext() == null) {
			KeydroidxLog.e("KeyProvider", "query 失败: getContext() == null");
			return null;
		}
		...
		if (match == CODE_SETTINGS || (uri.getPath() != null && uri.getPath().contains(PATH_SETTINGS))) {
			...
			KeydroidxSettingsStorage storage = new KeydroidxSettingsStorage(getContext());
		...
		int[] keyCodes = KeydroidxKeyBinding.loadKeyCodes(getContext());
		for (int i = 0; i < KeydroidxKeyBinding.ACTION_COUNT && i < keyCodes.length; i++) {
			int kc = keyCodes[i];
			String tag = (i < ACTION_TAGS.length) ? ACTION_TAGS[i] : ("ACTION_" + i);
			String name = KeydroidxKeyBinding.keyName(kc);
		...
		// 监听通知 URI
		cursor.setNotificationUri(getContext().getContentResolver(), uri);
		KeydroidxLog.i("KeyProvider", "成功返回按键映射，共 " + cursor.getCount() + " 项");
		return cursor;
	}
