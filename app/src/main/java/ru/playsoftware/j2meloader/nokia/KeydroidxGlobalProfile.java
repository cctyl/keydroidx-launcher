 ...
 */
public final class KeydroidxGlobalProfile {
	...
	private KeydroidxGlobalProfile() {
	}
	...
	public static void ensureGlobalProfile(Context context) {
		...
		if (!configFile.exists()) {
			...
			KeydroidxLog.i("GlobalProfile", "创建全局 profile 并注入桌面按键绑定");
		} else {
			KeydroidxLog.i("GlobalProfile", "全局 profile 已存在，跳过创建");
		}
		...
		if (!PROFILE_NAME.equals(prefs.getString(Constants.PREF_DEFAULT_PROFILE, null))) {
			...
			KeydroidxLog.i("GlobalProfile", "设置默认 profile = " + PROFILE_NAME);
		}
	}
	...
	public static boolean syncAppConfig(Context context, String name, String pathExt) {
		if (pathExt == null) {
			KeydroidxLog.w("GlobalProfile", "syncAppConfig: pathExt 为空，跳过 " + name);
			return false;
		}
		Context app = context.getApplicationContext();
		...
		if (grand == null) {
			KeydroidxLog.w("GlobalProfile", "syncAppConfig: 无法定位 workDir, pathExt=" + pathExt);
			return false;
		}
		...
		KeydroidxLog.i("GlobalProfile", "syncAppConfig: 已同步 " + name + " -> " + configDir.getAbsolutePath());
		return true;
	}
	...
	public static void syncKeyBindings(Context context) {
		...
		if (!profileDir.exists()) {
			KeydroidxLog.i("GlobalProfile", "全局 profile 不存在，先创建");
			ensureGlobalProfile(context);
			return;
		}
		...
		if (current == null) {
			...
			KeydroidxLog.i("GlobalProfile", "同步桌面绑定到全局 profile（current=null，跟随桌面）");
			return;
		}
		if (equalMaps(current, desktopNow)) {
			// 已与桌面一致，刷新同步标记
			prefs.edit().putString(PREF_SYNC_KEY, desktopJson).apply();
			KeydroidxLog.i("GlobalProfile", "全局按键映射已与桌面一致，无需修改");
			return;
		}
		if (lastSynced != null && serialize(current).equals(lastSynced)) {
			...
			ProfilesManager.saveConfig(params);
			prefs.edit().putString(PREF_SYNC_KEY, desktopJson).apply();
			KeydroidxLog.i("GlobalProfile", "桌面绑定已变更，更新全局 profile 按键映射");
			return;
		}
		// 用户手动改过 → 尊重用户，停止同步，但记录当前值作为新基线
		prefs.edit().putString(PREF_SYNC_KEY, serialize(current)).apply();
		KeydroidxLog.i("GlobalProfile", "检测到用户手动修改了全局按键映射，停止自动同步");
	}
	...
	public static SparseIntArray buildDesktopKeyMappings(Context context) {
		...
		KeydroidxKeyBinding nkb = new KeydroidxKeyBinding(context);
		inject(nkb, KeydroidxKeyBinding.ACTION_UP, Canvas.KEY_UP, map);
		inject(nkb, KeydroidxKeyBinding.ACTION_DOWN, Canvas.KEY_DOWN, map);
		inject(nkb, KeydroidxKeyBinding.ACTION_LEFT, Canvas.KEY_LEFT, map);
		inject(nkb, KeydroidxKeyBinding.ACTION_RIGHT, Canvas.KEY_RIGHT, map);
		inject(nkb, KeydroidxKeyBinding.ACTION_SELECT, Canvas.KEY_FIRE, map);
		inject(nkb, KeydroidxKeyBinding.ACTION_SOFT_LEFT, Canvas.KEY_SOFT_LEFT, map);
		inject(nkb, KeydroidxKeyBinding.ACTION_SOFT_RIGHT, Canvas.KEY_SOFT_RIGHT, map);
		return map;
	}

	private static void inject(KeydroidxKeyBinding nkb, int action, int midpKey, SparseIntArray map) {
