...
public class KeydroidxS60IconMap {
	...
	public static void loadFromDisk(Context context) {
		...
		try {
			...
			KeydroidxLog.w("S60IconMap", "loadFromDisk 失败: " + e.getMessage());
		}
		long elapsed = System.currentTimeMillis() - start;
		KeydroidxLog.i("S60IconMap", "loadFromDisk 完成：intentCache=" + intentCache.size() + " 项, lastPkgs="
	...
	public static void initAsync(Context context, final Runnable onComplete) {
		...
		synchronized (KeydroidxS60IconMap.class) {
			if (scanStarted) {
				// 扫描已在后台进行，不重复启动线程；回调直接派发（用当前内存缓存即可）
				KeydroidxLog.d("S60IconMap", "initAsync: 扫描进行中，跳过重复启动");
				if (onComplete != null) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					KeydroidxLog.w("S60IconMap", "initAsync 扫描异常: " + e.getMessage());
				} finally {
					scanStarted = false;
				}
				long elapsed = System.currentTimeMillis() - start;
				KeydroidxLog.i("S60IconMap", "initAsync 后台扫描结束，耗时 " + elapsed + "ms");
				if (onComplete != null) {
					mainHandler.post(onComplete);
				}
			}
	...
	private static void persistToDisk() {
		...
		try {
			...
			ed.commit();
			KeydroidxLog.i("S60IconMap", "persistToDisk: 已写入 " + intentCache.size() + " 项意图缓存");
		} catch (Exception e) {
			KeydroidxLog.w("S60IconMap", "persistToDisk 失败: " + e.getMessage());
		}
	}
	...
	public static void init(PackageManager pm) {
		...
		if (lastKnownPackages != null && lastKnownPackages.equals(currentPkgs)) {
			KeydroidxLog.d("S60IconMap", "init: 应用列表未变化，复用意图缓存 (" + intentCache.size() + " 项)");
			return;
		}
		...
		persistToDisk();
		long elapsed = System.currentTimeMillis() - start;
		KeydroidxLog.i("S60IconMap", "init: 意图扫描完成，缓存 " + intentCache.size() + " 项，耗时 " + elapsed + "ms");
	}
		...
		for (Object[] entry : LABEL_FALLBACK) {
			...
			for (int i = 1; i < entry.length; i++) {
				...
				if (lower.contains(keyword.toLowerCase(Locale.ROOT))) {
					KeydroidxLog.d("S60IconMap", "label 命中: '" + label + "' 含关键词 '" + keyword
							+ "' → resId=" + iconRes);
					return iconRes;
				}
			}
	...
	/**
	 * 根据 KeydroidxAppItem 中的包名反查是否匹配 S60 图标。
	 * 用于排序：能匹配到图标的排在前面。
	 */
	@DrawableRes
	public static int getIconForItem(KeydroidxAppItem item) {
