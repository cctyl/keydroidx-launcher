...
public class KeydroidxWidgetTilePickerFragment extends KeydroidxListPageFragment {
	...
	private void buildListUI() {
		...
		for (int i = 0; i < count; i++) {
			...
			row.setPadding(KeydroidxDimens.dp(getResources(), 8),
					KeydroidxDimens.dp(getResources(), 6),
					KeydroidxDimens.dp(getResources(), 8),
					KeydroidxDimens.dp(getResources(), 6));
			...
			ivIcon.setLayoutParams(new LinearLayout.LayoutParams(
					KeydroidxDimens.dp(getResources(), 24),
					KeydroidxDimens.dp(getResources(), 24)));
			...
			lpText.leftMargin = KeydroidxDimens.dp(getResources(), 8);
	...
	private void selectTile(TileInfo info) {
		if (info == null) return;
		KeydroidxWidgetStorage storage = new KeydroidxWidgetStorage(requireContext());
		KeydroidxWidgetItem item = new KeydroidxWidgetItem(
		...
		KeydroidxLog.i("WidgetTilePicker", "添加快捷开关组件: " + info.getDisplayTitle() + " target=" + info.getComponentValue());
