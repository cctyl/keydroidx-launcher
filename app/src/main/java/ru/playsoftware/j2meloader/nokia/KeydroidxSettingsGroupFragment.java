 ...
 */
public class KeydroidxSettingsGroupFragment extends KeydroidxListPageFragment {
	...
	private static String[] iconsOf(int group) {
		switch (group) {
			...
			case GROUP_CONTENT:
				return new String[]{
						KeydroidxIcons.ICON_SHORTCUTS,    // 顶部快捷栏设置
						KeydroidxIcons.ICON_WIDGETS,      // 桌面组件设置
						KeydroidxIcons.ICON_TOGGLES,      // 快捷开关
				};
			case GROUP_SYSTEM:
				return new String[]{
						KeydroidxIcons.ICON_LOG,          // 日志记录
	...
	public static KeydroidxSettingsGroupFragment newInstance(int group) {
		KeydroidxSettingsGroupFragment f = new KeydroidxSettingsGroupFragment();
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		for (int i = 0; i < itemNames.length; i++) {
			...
			ivIcon.setImageDrawable(KeydroidxIcons.get(requireContext(), itemIcons[i], 0xFFFFFFFF, 20));
			row.addView(ivIcon);
		...
		KeydroidxLog.i("SettingsGroup", "分组页初始化完成: " + getGroupTitle(group));
	}

	/** 取列表项名称：字体大小显示当前档位；日志记录显示开关状态；默认桌面根据状态动态展示。 */
	private String getItemDisplayName(int index) {
		if (group == GROUP_APPEARANCE && index == 0) {
			float cur = KeydroidxSettingsStorage.getFontScale(requireContext());
			String label = null;
		...
		if (group == GROUP_SYSTEM && index == 1) {
			boolean isDefault = ((KeydroidxDesktopActivity) requireActivity()).isDefaultLauncher();
			return isDefault ? "默认桌面：已设置" : "默认桌面设置";
		}
		if (group == GROUP_SYSTEM && index == 2) {
			return KeydroidxMusicSessionReader.isNotificationListenerEnabled(requireContext())
					? "通知中心：已授权" : "通知中心：未授权";
		}
	...
	public boolean onSelect() {
		KeydroidxLog.d("SettingsGroup", "onSelect 当前 focusIndex=" + focusIndex);
	...
	private boolean onSelectAppearance(KeydroidxDesktopActivity host) {
		switch (focusIndex) {
			case 0:
				KeydroidxLog.i("SettingsGroup", "字体大小设置");
				showFontScaleDialog();
				return true;
			case 1:
				KeydroidxLog.i("SettingsGroup", "进入字体选择");
				host.openFragment(new KeydroidxFontSettingsFragment());
				return true;
			case 2:
				KeydroidxLog.i("SettingsGroup", "进入主题设置");
				host.openFragment(new KeydroidxThemeSettingsFragment());
				return true;
			case 3:
	...
	private boolean onSelectKeys(KeydroidxDesktopActivity host) {
		switch (focusIndex) {
			case 0:
				KeydroidxLog.i("SettingsGroup", "按键绑定");
				host.openFragment(new KeydroidxKeyBindFragment());
				return true;
			case 1:
				KeydroidxLog.i("SettingsGroup", "进入应用向导");
	...
	private boolean onSelectContent(KeydroidxDesktopActivity host) {
		switch (focusIndex) {
			case 0:
				KeydroidxLog.i("SettingsGroup", "进入快捷栏设置");
				host.openFragment(new KeydroidxShortcutSettingsFragment());
				return true;
			case 1:
				KeydroidxLog.i("SettingsGroup", "进入桌面组件设置");
				host.openFragment(new KeydroidxWidgetSettingsFragment());
				return true;
			case 2:
				KeydroidxLog.i("SettingsGroup", "进入快捷开关设置");
				host.openFragment(new KeydroidxQuickToggleSettingsFragment());
				return true;
			default:
				return false;
		}
	...
	private boolean onSelectSystem(KeydroidxDesktopActivity host) {
		switch (focusIndex) {
			case 0:
				KeydroidxLog.i("SettingsGroup", "日志记录开关");
				toggleLogEnabled();
				return true;
			case 1:
				KeydroidxLog.i("SettingsGroup", "默认桌面设置：引导设为默认桌面");
				host.requestSetDefaultLauncher();
				return true;
			case 2:
				KeydroidxLog.i("SettingsGroup", "进入通知中心设置");
				host.openFragment(new KeydroidxNotificationSettingsFragment());
				return true;
			case 3:
	...
	private void toggleLogEnabled() {
		boolean next = !KeydroidxSettingsStorage.isFileLogEnabled(requireContext());
		KeydroidxSettingsStorage.setFileLogEnabled(requireContext(), next);
	...
	private void showFontScaleDialog() {
		...
		for (int i = 0; i < FONT_LABELS.length; i++) {
			...
			String itemLabel = Math.abs(scale - cur) < 0.001f
					? label + "（当前）" : label;
			items.add(new KeydroidxOptionsDialog.OptionItem(0, itemLabel, true, false, () -> {
				KeydroidxSettingsStorage.setFontScale(requireContext(), scale);
				KeydroidxLog.i("SettingsGroup", "字体大小已设置: " + label + " scale=" + scale);
				((KeydroidxDesktopActivity) requireActivity()).recreate();
			}));
		}
		// 增加自定义倍率选项（支持手动输入 0.5 ~ 3.5x）
		items.add(new KeydroidxOptionsDialog.OptionItem(0, "自定义倍率...", true, false, () -> {
			KeydroidxDesktopActivity host = (KeydroidxDesktopActivity) requireActivity();
			host.openFragment(new KeydroidxCustomFontScaleFragment());
		}));
		KeydroidxOptionsDialog.show(getParentFragmentManager(), "字体大小", items);
	}
