 ...
 */
public class KeydroidxDesktopKeepAliveService extends Service {
	...
	private boolean startForegroundSafe(Notification notification) {
		try {
			...
			KeydroidxLog.w(TAG, "startForeground 失败，保活降级（桌面不受影响）: " + t);
			return false;
		}
	}
	...
	private Notification buildNotification() {
		...
		backIntent.setClass(this, KeydroidxDesktopActivity.class);
	...
	public static void start(Context context) {
		Intent intent = new Intent(context, KeydroidxDesktopKeepAliveService.class);
		try {
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
			...
			KeydroidxLog.w(TAG, "启动保活服务失败（忽略，桌面不受影响）: " + e);
		}
	}
}
