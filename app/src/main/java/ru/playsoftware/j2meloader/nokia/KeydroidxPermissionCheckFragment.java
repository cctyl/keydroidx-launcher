...
public class KeydroidxPermissionCheckFragment extends KeydroidxListPageFragment {
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		rows.add(new Row(KeydroidxIcons.ICON_APP, "应用列表", ID_APP_LIST));
		rows.add(new Row(KeydroidxIcons.ICON_KEYPAD, "电话状态/信号", ID_PHONE));
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
			rows.add(new Row(KeydroidxIcons.ICON_NOTIFICATIONS, "允许通知", ID_NOTIF));
		}
		rows.add(new Row(KeydroidxIcons.ICON_NOTIFICATIONS_OFF, "允许读取通知", ID_LISTENER));
		rows.add(new Row(KeydroidxIcons.ICON_REFRESH, "一键修复全部缺失项", ID_FIX_ALL));
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			ivIcon.setImageDrawable(KeydroidxIcons.get(requireContext(), rows.get(i).icon, 0xFFFFFFFF, 20));
			row.addView(ivIcon);
		...
		refreshStatuses();
		setFocusIndex(0);
		KeydroidxLog.i(TAG, "系统权限自检页初始化完成，共 " + rows.size() + " 项");
	}

	/** 刷新各行状态文案（授权后回调调用）。 */
	private void refreshStatuses() {
		boolean appList = KeydroidxPermissionManager.hasAppListPermission(requireContext());
		boolean phone = KeydroidxPermissionManager.isGranted(requireContext(), Permission.READ_PHONE_STATE);
		boolean notif = Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU
				|| KeydroidxPermissionManager.isGranted(requireContext(), Permission.POST_NOTIFICATIONS);
		boolean listener = KeydroidxPermissionManager.isGranted(requireContext(), Permission.BIND_NOTIFICATION_LISTENER_SERVICE);
	...
	public boolean onSelect() {
		...
		KeydroidxLog.i(TAG, "onSelect id=" + r.id + " name=" + r.name);
		switch (r.id) {
			case ID_APP_LIST:
				if (KeydroidxPermissionManager.hasAppListPermission(requireContext())) {
					toastShort("该项已授权");
				} else {
					KeydroidxPermissionManager.requestAppListPermission(host,
							"需要应用列表权限以展示与启动应用", makeCallback(host));
				}
				return true;
			case ID_PHONE:
				if (KeydroidxPermissionManager.isGranted(requireContext(), Permission.READ_PHONE_STATE)) {
					toastShort("该项已授权");
				} else {
					KeydroidxPermissionManager.request(host, makeCallback(host), Permission.READ_PHONE_STATE);
				}
				return true;
			case ID_NOTIF:
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU
						&& !KeydroidxPermissionManager.isGranted(requireContext(), Permission.POST_NOTIFICATIONS)) {
					KeydroidxPermissionManager.request(host, makeCallback(host), Permission.POST_NOTIFICATIONS);
				} else {
					toastShort("该项已授权");
				}
				return true;
			case ID_LISTENER:
				if (KeydroidxPermissionManager.isGranted(requireContext(), Permission.BIND_NOTIFICATION_LISTENER_SERVICE)) {
					toastShort("该项已授权");
				} else {
					KeydroidxPermissionManager.request(host, makeCallback(host), Permission.BIND_NOTIFICATION_LISTENER_SERVICE);
				}
				return true;
			case ID_FIX_ALL:
				if (KeydroidxPermissionManager.isCorePermissionsGranted(requireContext())) {
					toastShort("全部权限已就绪");
				} else {
					KeydroidxPermissionManager.requestCorePermissions(host,
	...
	private OnPermissionCallback makeCallback(KeydroidxDesktopActivity host) {
