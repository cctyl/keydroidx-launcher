...
public class KeydroidxKeyBinding {
	...
	public void markWizardDone() {
		...
		KeydroidxLog.i("KeyBinding", "首次启动按键绑定向导已标记为完成");
	}
	...
	public KeydroidxKeyBinding(Context context) {
		this.context = context.getApplicationContext();
		prefs = this.context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
		load();
		KeydroidxLog.i("KeyBinding", "初始化完成，当前绑定：");
		dumpBindings();
	}
	...
	private void dumpBindings() {
		for (int i = 0; i < ACTION_COUNT; i++) {
			int kc = keycodes[i];
			String state = KeydroidxKeyBinding.isBound(kc)
					? keyName(kc)
					: "未绑定";
			KeydroidxLog.i("KeyBinding", "  " + getActionName(i) + " -> " + state);
		}
	}
	...
	public void reload() {
		...
		if (changed) {
			KeydroidxLog.i("KeyBinding", "绑定已变更，重新加载：");
			dumpBindings();
		}
	}
	...
	public void setKeyCode(int action, int keycode) {
		if (action < 0 || action >= ACTION_COUNT) {
			KeydroidxLog.w("KeyBinding", "setKeyCode 忽略非法 action=" + action);
			return;
		}
		...
		if (keycode != KeyEvent.KEYCODE_UNKNOWN) {
			for (int i = 0; i < ACTION_COUNT; i++) {
				if (i != action && keycodes[i] == keycode) {
					...
					KeydroidxLog.i("KeyBinding", "清除冲突绑定 "
	...
	private void notifyProviderChanged() {
		try {
			if (context != null) {
				...
				KeydroidxLog.d("KeyBinding", "已发送按键变更通知: " + uri);
			}
		} catch (Exception e) {
			KeydroidxLog.w("KeyBinding", "notifyProviderChanged 失败: " + e.getMessage());
		}
	}
	...
	public int resolveAction(KeyEvent event) {
		if (event.getAction() != KeyEvent.ACTION_DOWN) {
			KeydroidxLog.d("KeyBinding", "resolveAction 忽略非按下事件 action="
		...
		if (keyCode == KeyEvent.KEYCODE_DPAD_CENTER
			...
			KeydroidxLog.i("KeyBinding", "resolveAction 通用确认键兜底 "
		...
		if (keyCode == KeyEvent.KEYCODE_MENU) {
			KeydroidxLog.i("KeyBinding", "resolveAction 菜单键兜底 "
	 ...
	 * 供无法取得 KeydroidxKeyBinding 实例的场景使用：如 :midlet 进程内的弹窗
	 * （KeydroidxOptionsDialog 注入键码表模式，MicroActivity 宿主不是 KeydroidxDesktopActivity）。
	 ...
	 * 原在 KeydroidxLog 中；KeydroidxLog 合并到 common 后迁入本类——按键语义归属按键绑定类更合理。
	 */
