 ...
 * <ul>
 * <li>订阅 {@link KeydroidxNotificationRepository} 并重建列表（保持焦点位置）；</li>
 * <li>行视图构建（图标 + 应用名/时间 + 标题 + 摘要 + 未读点）；</li>
 * <li>打开通知（contentIntent.send，失败不崩溃）与清除动作。</li>
 * </ul>
 */
public class KeydroidxNotificationCenterFragment extends KeydroidxListPageFragment
		implements KeydroidxNotificationRepository.Listener {
	...
	private List<KeydroidxNotificationItem> current = new ArrayList<>();
	...
	public void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		emptyIcon.setText(KeydroidxIcons.NOTIFICATIONS);

		listScroll = view.findViewById(R.id.notifCenterScroll);
		constrainScrollHeight(view, listScroll);

		KeydroidxNotificationRepository.get().addListener(this);
		rebuildList(null);
	}
	...
	public void onResume() {
		...
		// 进入页面即视为已读
		KeydroidxNotificationRepository.get().markAllRead();
	}

	@Override
	public void onDestroyView() {
		KeydroidxNotificationRepository.get().removeListener(this);
	...
	private void rebuildList(@Nullable String keepFocusKey) {
		...
		current = KeydroidxNotificationRepository.get().getItems();
		boolean granted = KeydroidxMusicSessionReader.isNotificationListenerEnabled(requireContext());
		boolean connected = KeydroidxNotificationRepository.get().isConnected();
		...
		for (int i = 0; i < current.size(); i++) {
			KeydroidxNotificationItem item = current.get(i);
	...
	private View buildRow(final KeydroidxNotificationItem item, final int index) {
		int dp4 = KeydroidxDimens.dp(getResources(), 4);
		...
		if (item.icon != null) {
			...
			ivIcon.setImageDrawable(KeydroidxIcons.get(requireContext(),
					KeydroidxIcons.ICON_NOTIFICATIONS, 0xFF9DB4E0, 16));
		}
		row.addView(ivIcon);
		...
		KeydroidxFontManager.textSize(tvTime, 9);
		...
		KeydroidxFontManager.textSize(tvTitle, 12);
	...
	public boolean onSelect() {
		...
		final KeydroidxNotificationItem item = current.get(focusIndex);
		...
		try {
			...
			KeydroidxLog.i("NotifCenter", "打开通知: " + item.pkg + " " + item.displayTitle());
		} catch (PendingIntent.CanceledException e) {
			KeydroidxLog.w("NotifCenter", "打开通知失败: " + e.getMessage());
			Toast.makeText(requireContext(), "无法打开该通知", Toast.LENGTH_SHORT).show();
		} catch (Exception e) {
			KeydroidxLog.e("NotifCenter", "打开通知异常", e);
			Toast.makeText(requireContext(), "无法打开该通知", Toast.LENGTH_SHORT).show();
		}
		return true;
	}
	...
	private void showOptionsMenu() {
		...
		if (getParentFragmentManager().findFragmentByTag("KeydroidxOptions") != null) {
			return;
		}
		...
		if (!granted) {
			items.add(new KeydroidxOptionsDialog.OptionItem(KeydroidxIcons.ICON_SETTINGS, "去授权", true, false,
					() -> {
						if (!KeydroidxMusicSessionReader.openNotificationListenerSettings(requireContext())) {
			...
			final KeydroidxNotificationItem item = current.get(focusIndex);
			items.add(new KeydroidxOptionsDialog.OptionItem(KeydroidxIcons.ICON_DELETE, "清除", item.clearable,
					false, () -> clearOne(item)));
		}
		...
		KeydroidxOptionsDialog.show(getParentFragmentManager(), "通知中心", items);
	}

	/** 单条清除：走服务 cancel；随后强制按系统实际通知列表刷新，不依赖移除回调是否送达。 */
	private void clearOne(KeydroidxNotificationItem item) {
		...
		if (ok) {
			KeydroidxLog.i("NotifCenter", "已清除通知: " + item.pkg + " " + item.displayTitle());
