 ...
 */
public class KeydroidxWidgetActivityNameFragment extends KeydroidxPageFragment {
	...
	public static KeydroidxWidgetActivityNameFragment newAddMode(String packageName,
			String className, String activityLabel) {
		KeydroidxWidgetActivityNameFragment f = new KeydroidxWidgetActivityNameFragment();
		Bundle b = new Bundle();
	...
	public static KeydroidxWidgetActivityNameFragment newEditMode(String packageName,
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		KeydroidxLog.i(TAG, "预填名称: \"" + prefill + "\"");

		// IME done → 关闭软键盘
		etName.setOnEditorActionListener((v, actionId, event) -> {
			if (actionId == EditorInfo.IME_ACTION_DONE) {
				KeydroidxLog.d(TAG, "IME done → 关闭软键盘");
				exitEditing();
				applyFocus();
				return true;
			}
	...
	private void activateEditing() {
		...
		if (imm != null) {
			...
			KeydroidxLog.d(TAG, "激活编辑态，软键盘弹出");
		}
		applyFocus();
	}
	...
	private void applyFocus() {
		...
		if (focusIndex == FOCUS_SAVE) {
			...
			tvSaveButton.setBackground(KeydroidxTheme.createSelectionDrawable(requireContext(), 4));
		} else {
	...
	public boolean onSoftRight() {
		...
		// 右软键「返回」：回到步骤2
		KeydroidxLog.i(TAG, "右软键：返回步骤2（选择Activity）");
	...
	private void validateAndSave() {
		...
		KeydroidxLog.i(TAG, "保存 name=\"" + name + "\"");
		...
		KeydroidxWidgetItem item = new KeydroidxWidgetItem(KeydroidxWidgetItem.TYPE_ACTIVITY, name, payload, packageName);
		...
		if (MODE_EDIT.equals(mode)) {
			...
			if (entries >= 3) {
				...
				fm.popBackStackImmediate();
				fm.popBackStackImmediate();
				KeydroidxLog.i(TAG, "EDIT 保存后同步出栈 3 层");
			} else {
				fm.popBackStackImmediate();
				KeydroidxLog.i(TAG, "EDIT 保存后出栈 1 层（返回栈不足3层）");
			}
		} else {
			if (storage.isFull()) {
				showToast("组件已达上限");
			...
			if (entries >= 4) {
				...
				fm.popBackStackImmediate();
				fm.popBackStackImmediate();
				KeydroidxLog.i(TAG, "ADD 保存后同步出栈 4 层");
			} else if (entries >= 3) {
				fm.popBackStackImmediate();
				fm.popBackStackImmediate();
				fm.popBackStackImmediate();
				KeydroidxLog.i(TAG, "ADD 保存后出栈 3 层（返回栈不足4层）");
			} else {
				fm.popBackStackImmediate();
				KeydroidxLog.i(TAG, "ADD 保存后出栈 1 层（返回栈不足3层）");
			}
		}
	}
