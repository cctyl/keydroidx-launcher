 ...
 * S6（类型选择）由 {@link KeydroidxWidgetTypePickerFragment} 实现。
 * <p>
 * S2 选项菜单与 S3 删除子菜单均为底部弹出的 DialogFragment
 * （样式与「功能表→应用程序→选中JAR的选项菜单」一致），弹窗内已接入 KeydroidxKeyBinding。
 */
public class KeydroidxWidgetSettingsFragment extends KeydroidxPageFragment {
	...
	private final List<KeydroidxWidgetItem> widgets = new ArrayList<>();
	...
	public void onResume() {
		...
		KeydroidxLog.i(TAG, "onResume 重载组件列表，数量=" + widgets.size());
		rebuildList();
		updateBottomBar();
	}
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		widgets.clear();
		widgets.addAll(storage.getWidgets());
		KeydroidxLog.i(TAG, "桌面组件设置初始化完成，组件数=" + widgets.size());
	...
	private void onConfirmWidget() {
		...
		KeydroidxWidgetItem item = widgets.get(focusIndex);
		if (item.isEditable()) {
			if (item.type == KeydroidxWidgetItem.TYPE_APP) {
				// 应用类组件：进入应用选择页（编辑模式），换绑应用
				KeydroidxLog.i(TAG, "确认键：应用类组件进入编辑（换绑） label=" + item.label);
				((KeydroidxDesktopActivity) requireActivity())
						.openFragment(KeydroidxWidgetAppPickerFragment.newEditMode(focusIndex));
			} else if (item.type == KeydroidxWidgetItem.TYPE_URL) {
				// 网址类组件：进入网址编辑页（编辑模式）
				KeydroidxLog.i(TAG, "确认键：网址类组件进入编辑 label=" + item.label);
				((KeydroidxDesktopActivity) requireActivity())
						.openFragment(KeydroidxWidgetUrlEditFragment.newEditMode(focusIndex));
			} else if (item.type == KeydroidxWidgetItem.TYPE_ACTIVITY) {
				// Activity快捷组件：进入步骤1应用选择页（编辑模式），从头开始选
				// 详见 docs/6-Activity快捷组件添加编辑界面设计.md
				KeydroidxLog.i(TAG, "确认键：Activity快捷组件进入编辑（从步骤1开始） label=" + item.label);
				((KeydroidxDesktopActivity) requireActivity())
						.openFragment(KeydroidxWidgetAppPickerFragment.newActivityEditMode(focusIndex));
			}
		} else {
			KeydroidxLog.i(TAG, "确认键：该组件不可编辑 label=" + item.label);
			showToast("该组件不可编辑");
		}
	}
	...
	private void showOptionsDialog() {
		boolean canAdd = widgets.size() < KeydroidxWidgetItem.MAX_COUNT;
		...
		items.add(new KeydroidxOptionsDialog.OptionItem(KeydroidxIcons.ICON_ADD,
		...
		items.add(new KeydroidxOptionsDialog.OptionItem(KeydroidxIcons.ICON_SORT,
				"组件排序", canSort, false, this::enterSortMode));
		KeydroidxOptionsDialog.show(getParentFragmentManager(), "选项", items);
		KeydroidxLog.i(TAG, "弹出选项菜单弹窗: canAdd=" + canAdd + " canDelete=" + canDelete + " canSort=" + canSort);
	}

	private void openTypePicker() {
		KeydroidxLog.i(TAG, "打开组件类型选择页（S6）");
		((KeydroidxDesktopActivity) requireActivity()).openFragment(new KeydroidxWidgetTypePickerFragment());
	}
	...
	private void enterDeleteMode() {
		...
		rebuildList();
		updateBottomBar();
		KeydroidxLog.i(TAG, "进入删除模式，共 " + widgets.size() + " 项");
	}

	private void showDeleteDialog() {
		// 全选/取消全选项 keepOpen=true，点击后不关闭并刷新文案（删除已选按钮计数随之更新）
		final KeydroidxOptionsDialog[] holder = new KeydroidxOptionsDialog[1];
		List<KeydroidxOptionsDialog.OptionItem> items = buildDeleteDialogItems(holder);
		holder[0] = KeydroidxOptionsDialog.show(getParentFragmentManager(), "删除", items);
		KeydroidxLog.i(TAG, "弹出删除子菜单弹窗");
	}

	private List<KeydroidxOptionsDialog.OptionItem> buildDeleteDialogItems(final KeydroidxOptionsDialog[] holder) {
		boolean all = allChecked();
		int count = checkedCount();
		List<KeydroidxOptionsDialog.OptionItem> items = new ArrayList<>();
		items.add(new KeydroidxOptionsDialog.OptionItem(all ? KeydroidxIcons.ICON_CHECK_BOX_OUTLINE_BLANK : KeydroidxIcons.ICON_CHECK_BOX,
				all ? "取消全选" : "全选",
				true, true, () -> {
			toggleSelectAll();
	...
	private void toggleChecked(int index) {
		...
		KeydroidxLog.d(TAG, "切换勾选 index=" + index + " -> " + checked.get(index));
	}
	...
	private void toggleSelectAll() {
		...
		rebuildList();
		KeydroidxLog.i(TAG, "全选/取消全选: 当前勾选 " + checkedCount() + " 项");
	}

	private void deleteSelected() {
		List<KeydroidxWidgetItem> toRemove = new ArrayList<>();
		for (int i = 0; i < checked.size(); i++) {
			if (checked.get(i)) {
		...
		if (toRemove.isEmpty()) {
			KeydroidxLog.w(TAG, "删除已选：无勾选项");
			return;
		}
		KeydroidxLog.i(TAG, "删除已选组件 " + toRemove.size() + " 个");
		storage.removeWidgets(toRemove);
		widgets.removeAll(toRemove);
		KeydroidxLog.i(TAG, "删除后剩余组件 " + widgets.size() + " 个");
		backToNormal();
	}
	...
	private void liftCurrent() {
		...
		applyListHighlight();
		KeydroidxLog.i(TAG, "拎起行 index=" + liftedIndex);
	}
	...
	private void dropLifted() {
		...
		liftedIndex = -1;
		applyListHighlight();
		KeydroidxLog.i(TAG, "放下行 -> 光标落回 index=" + focusIndex);
	}

	private void swapWidgets(int a, int b) {
		KeydroidxWidgetItem tmp = widgets.get(a);
		widgets.set(a, widgets.get(b));
		widgets.set(b, tmp);
		KeydroidxLog.d(TAG, "交换行 " + a + " <-> " + b);
	}
	...
	private void finishSort() {
		...
		KeydroidxLog.i(TAG, "保存组件排序并退出排序模式");
		backToNormal();
	}
	...
	private boolean onLiftedDirection(int direction) {
		...
		if (direction == KeydroidxKeyBinding.ACTION_UP && liftedIndex > 0) {
		...
		if (direction == KeydroidxKeyBinding.ACTION_DOWN && liftedIndex < widgets.size() - 1) {
			swapWidgets(liftedIndex, liftedIndex + 1);
			liftedIndex++;
			rebuildList();
			applyListHighlight();
	...
	private void rebuildList() {
		...
		if (widgets.isEmpty()) {
			...
			KeydroidxFontManager.textSize(empty, 12);
		...
		for (int i = 0; i < widgets.size(); i++) {
			KeydroidxWidgetItem item = widgets.get(i);
			...
			if (mode == MODE_DELETE) {
				...
				tvCheck.setLayoutParams(new LinearLayout.LayoutParams(KeydroidxDimens.dp(getResources(), 24), KeydroidxDimens.dp(getResources(), 24)));
				tvCheck.setGravity(Gravity.CENTER);
				KeydroidxFontManager.textSize(tvCheck, 13);
		...
		iv.setLayoutParams(new LinearLayout.LayoutParams(KeydroidxDimens.dp(getResources(), 20), KeydroidxDimens.dp(getResources(), 20)));
		if (item.type == KeydroidxWidgetItem.TYPE_APP) {
			// 应用组件：加载真实应用图标（S60 → 系统图标 → 占位），未命中则后台异步刷新
			loadAppIcon(item, iv);
		} else {
			iv.setImageDrawable(KeydroidxIcons.get(requireContext(), item.getTypeIconUnicode(), 0xFFFFFFFF, 20));
		}
		row.addView(iv);

			// 间距
			row.addView(spaceView(KeydroidxDimens.dp(getResources(), 6), 1));
			...
			KeydroidxFontManager.textSize(tvTag, 9);
			row.addView(tvTag);
	 ...
	 */
	private void loadAppIcon(KeydroidxWidgetItem item, final ImageView iv) {
		...
		if (cn == null) {
			...
			try {
				iv.setImageDrawable(ContextCompat.getDrawable(requireContext(),
						KeydroidxWidgetItem.getTypeIcon(KeydroidxWidgetItem.TYPE_APP)));
			} catch (Exception ignored) {
				KeydroidxLog.w(TAG, "加载应用组件占位图标失败");
			}
			return;
		}
	...
	private ComponentName parseAppValue(String value) {
		...
		try {
			...
			KeydroidxLog.w(TAG, "解析应用组件 value 失败: " + value);
			return null;
		}
	}

	/** 后台线程加载系统真实图标，完成后回主线程更新 ImageView。 */
	private void loadAppIconAsync(final KeydroidxWidgetItem item, final ComponentName cn, final ImageView iv) {
			...
			public void run() {
				...
				new Handler(Looper.getMainLooper()).post(new Runnable() {
					...
					public void run() {
						...
						KeydroidxLog.d(TAG, "应用组件 " + item.label + " 使用系统真实图标");
					}
				});
			}
	...
	private void updateStatusText() {
		...
		switch (mode) {
			...
			case MODE_SORT:
				...
				break;
			default:
				tvStatus.setText("已选 " + widgets.size() + " / " + KeydroidxWidgetItem.MAX_COUNT + " 项");
				break;
		}
	}

	private void updateBottomBar() {
		// 底部菜单栏由 KeydroidxPage 声明 + host.refreshPageBar() 自动装配（按 mode 动态取值）
	...
	private void applyListHighlight() {
		...
		if (mode == MODE_SORT && lifted && liftedIndex >= 0 && liftedIndex < itemViews.length) {
			...
			focusRow.setBackground(KeydroidxTheme.createSelectionDrawable(requireContext(), 4));
			LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) focusRow.getLayoutParams();
			if (lp != null && lp.height != KeydroidxDimens.dp(getResources(), 34)) {
				lp.height = KeydroidxDimens.dp(getResources(), 34);
