 ...
 *   <li>「缩放模式」：居中裁剪 / 拉伸铺满 / 适应屏幕 三选一（复用 {@link KeydroidxOptionsDialog}）；
 *   <li>「恢复默认」：删除自定义壁纸，回退到当前主题的渐变背景。
 * </ul>
 * 背景机制见 {@link KeydroidxWallpaper}：全屏壁纸由 Activity 统一承载，
 * 因此设置后<b>桌面、功能表、百宝箱、设置等所有页面同步生效</b>。
 * <p>
 * 注意：系统文件选择器只能用触屏操作（与「字体设置 → 导入字体」一致），
 * 方向键在本页仅用于切换列表项。
 */
public class KeydroidxWallpaperSettingsFragment extends KeydroidxListPageFragment {

	private static final String TAG = "WallpaperSettings";
	private static final int REQUEST_PICK_IMAGE = 1002;

	/** 缩放模式文案（顺序与 {@link KeydroidxSettingsStorage#WALLPAPER_SCALE_CROP} 等常量一致）。 */
	...
	private void rebuildList() {
		...
		final boolean hasCustom = KeydroidxWallpaper.hasCustomWallpaper(requireContext());
		final int scaleMode = KeydroidxSettingsStorage.getWallpaperScale(requireContext());
		...
		String[] icons = {
				KeydroidxIcons.ICON_WALLPAPER,  // 选择图片
				KeydroidxIcons.ICON_DISPLAY,    // 缩放模式
				KeydroidxIcons.ICON_RESTORE,    // 恢复默认
		};
		String[] names = {
				"选择图片",
	...
	private LinearLayout buildRow(String iconUnicode, String name, int rowHeight) {
		...
		ivIcon.setImageDrawable(KeydroidxIcons.get(requireContext(), iconUnicode, 0xFFFFFFFF, 20));
		row.addView(ivIcon);
	...
	private void pickImage() {
		try {
			...
			KeydroidxLog.e(TAG, "打开图片选择器失败", e);
			showToast("无法打开文件选择器");
		}
	}
	...
	public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
		...
		//    必须在这里同步完成 IO，再交给后台线程做解码（详见 KeydroidxWallpaper.copyToCacheSync）。
		showToast("正在导入壁纸...");
		final File cached = KeydroidxWallpaper.copyToCacheSync(appCtx, uri);
		if (cached == null) {
		...
		new Thread(() -> {
			final boolean ok = KeydroidxWallpaper.finalizeImport(appCtx, cached);
			mainHandler.post(() -> {
	...
	private void showScaleDialog() {
		final int cur = KeydroidxSettingsStorage.getWallpaperScale(requireContext());
		...
		for (int i = 0; i < SCALE_LABELS.length; i++) {
			...
			items.add(new KeydroidxOptionsDialog.OptionItem(KeydroidxIcons.ICON_WALLPAPER, label,
					true, false, () -> {
						KeydroidxSettingsStorage.setWallpaperScale(requireContext(), mode);
						KeydroidxLog.i(TAG, "壁纸缩放模式切换为: " + SCALE_LABELS[mode]);
						applyAndRefresh();
						showToast("已切换：" + SCALE_LABELS[mode]);
					}));
		}
		KeydroidxOptionsDialog.show(getParentFragmentManager(), "缩放模式", items);
	}

	/** 清除自定义壁纸，回退主题渐变背景。 */
	private void restoreDefault() {
		if (!KeydroidxWallpaper.hasCustomWallpaper(requireContext())) {
			showToast("当前已是主题背景");
			return;
		}
		KeydroidxWallpaper.clear(requireContext());
		applyAndRefresh();
		showToast("已恢复主题背景");
	}
	...
	private static String scaleLabel(int mode) {
		...
		return SCALE_LABELS[KeydroidxSettingsStorage.WALLPAPER_SCALE_CROP];
	}

	private int dp(float value) {
		return KeydroidxDimens.dp(getResources(), value);
	}
