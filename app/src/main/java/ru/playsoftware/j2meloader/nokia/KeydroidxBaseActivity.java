...
public abstract class KeydroidxBaseActivity extends AppCompatActivity {
	...
	protected void attachBaseContext(Context newBase) {
		...
		// 统一同步进 common KeydroidxFontManager 的单一缩放源 sFontScale / sCurrentFontId。
		// 严禁在此把 Configuration.fontScale 设为 userFontScale（会与 sFontScale 双重缩放），
		// 仅固定为 1.0 以剥离系统字体设置对 sp 字号的干扰，缩放完全交给 KeydroidxFontManager。
		...
		// 用户字体倍率完全由 KeydroidxFontManager.sFontScale 承担（见规范 §6.1）。
		if (fixed != dpi) {
	...
	protected void setupKeydroidxUi() {
		...
		if (bottomPanel != null) {
			KeydroidxFontManager.applyFontToViewHierarchy(bottomPanel);
		}
	}
	...
	private void applyBottomText(TextView tv, String text, boolean isCenter) {
		...
		if (text == null || text.isEmpty()) {
			...
			if (isCenter) {
				...
				KeydroidxFontManager.textSize(tv, size);
