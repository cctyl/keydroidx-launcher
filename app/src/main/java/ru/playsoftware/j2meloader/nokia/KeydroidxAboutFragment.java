...
/**
 * 桌面关于页：复用 common 的 {@link io.github.cctyl.nokia.common.ui.about.KeydroidxAboutFragment}
 * 统一页面（内容装配、检查更新、详细日志开关、点阵字体自适应等全部走 common），
 * 仅在此薄子类里覆写右软键/返回键，使其弹出设置栈。
 *
 * <p>原因：launcher 的 {@code KeydroidxDesktopActivity} 触摸右软键走
 * {@code dispatchActionToHost}，未消费时没有 exitCurrent 兜底；若沿用 common 默认
 * {@code onSoftRight()->onBack()->false}，触屏点「返回」不会弹栈。故显式调用
 * {@link KeydroidxDesktopActivity#exitCurrent()}，保留触屏与物理按键一致的返回体验。</p>
 *
 * <p>config 由 {@link KeydroidxDesktopSettingsFragment} 通过 {@link #newInstance(KeydroidxAboutConfig)}
 ...
 */
public class KeydroidxAboutFragment extends io.github.cctyl.nokia.common.ui.about.KeydroidxAboutFragment {

	public static KeydroidxAboutFragment newInstance(@Nullable KeydroidxAboutConfig config) {
		KeydroidxAboutFragment f = new KeydroidxAboutFragment();
		if (config != null) {
