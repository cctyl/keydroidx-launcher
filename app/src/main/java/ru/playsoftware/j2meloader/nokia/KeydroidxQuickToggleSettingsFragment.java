 ...
 */
public class KeydroidxQuickToggleSettingsFragment extends KeydroidxPageFragment {
	...
	private KeydroidxQuickToggleStorage storage;
	private List<KeydroidxQuickToggleItem> toggles = new ArrayList<>();
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		storage = new KeydroidxQuickToggleStorage(requireContext());
	...
	private void rebuildList() {
		...
		int padV = KeydroidxDimens.dp(getResources(), 6);
		int iconSize = KeydroidxDimens.dp(getResources(), 18);
		int iconMargin = KeydroidxDimens.dp(getResources(), 8);

		for (int i = 0; i < toggles.size(); i++) {
			final int index = i;
			final KeydroidxQuickToggleItem item = toggles.get(i);
			...
			// 与「快捷栏设置」页保持一致：复选框统一在行首，字号走 KeydroidxFontManager（dp 单位 + 全局像素字体）
			TextView checkTv = new TextView(getContext());
			KeydroidxFontManager.textSize(checkTv, 12);
			if (mode == MODE_SORT) {
				checkTv.setText((i + 1) + ".");
				checkTv.setTextColor(0xFF888888);
				checkTv.setMinWidth(KeydroidxDimens.dp(getResources(), 24));
			} else {
			...
			iv.setImageDrawable(KeydroidxIcons.get(getContext(), item.getIconUnicode(getContext()), iconColor, 18));
			row.addView(iv);
			...
			if (i < toggles.size() - 1) {
				...
				LinearLayout.LayoutParams lineLp = new LinearLayout.LayoutParams(
						ViewGroup.LayoutParams.MATCH_PARENT, KeydroidxDimens.dp(getResources(), 1));
				line.setLayoutParams(lineLp);
				line.setBackground(new KeydroidxDashedLineDrawable(getResources(), 0x40FFFFFF, 2, 2));
		...
		toggleScroll.post(new Runnable() {
			...
			public void run() {
				if (toggleScroll != null && target != null) {
					int scrollY = target.getTop() - KeydroidxDimens.dp(getResources(), 20);
	...
	public boolean onDirection(int direction) {
		...
		if (mode == MODE_SORT && lifted) {
			...
			} else if (direction == KeydroidxKeyBinding.ACTION_DOWN) {
	...
	public boolean onSelect() {
		...
		// 普通模式：快速切换勾选状态
		KeydroidxQuickToggleItem item = toggles.get(focusIndex);
	...
	private void showOptionsMenu() {
		final KeydroidxOptionsDialog[] dialogHolder = new KeydroidxOptionsDialog[1];
		List<KeydroidxOptionsDialog.OptionItem> items = buildOptionsItems(dialogHolder);
		dialogHolder[0] = KeydroidxOptionsDialog.show(getParentFragmentManager(), "选项", items);
	}

	private List<KeydroidxOptionsDialog.OptionItem> buildOptionsItems(final KeydroidxOptionsDialog[] dialogHolder) {
		List<KeydroidxOptionsDialog.OptionItem> items = new ArrayList<>();

		// 1. 调整排序
		items.add(new KeydroidxOptionsDialog.OptionItem(
				KeydroidxIcons.ICON_SORT,
				"调整排序",
				true,
				false,
				new Runnable() {
		...
		items.add(new KeydroidxOptionsDialog.OptionItem(
				all ? KeydroidxIcons.ICON_CHECK_BOX_OUTLINE_BLANK : KeydroidxIcons.ICON_CHECK_BOX,
				all ? "取消全选" : "全选",
				true,
				true,
				new Runnable() {
