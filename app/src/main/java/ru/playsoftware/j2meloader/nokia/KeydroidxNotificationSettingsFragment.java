 ...
 * </ul>
 * 复用 {@link KeydroidxListPageFragment} 的循环导航/焦点三件套与设置分组页布局。
 */
public class KeydroidxNotificationSettingsFragment extends KeydroidxListPageFragment {
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		for (int i = 0; i < NAMES.length; i++) {
			...
			ivIcon.setImageDrawable(KeydroidxIcons.get(requireContext(),
					KeydroidxIcons.ICON_NOTIFICATIONS, 0xFFFFFFFF, 20));
			row.addView(ivIcon);
	...
	private void refreshNames() {
		...
		tvNames[0].setText(KeydroidxMusicSessionReader.isNotificationListenerEnabled(requireContext())
				? "通知使用权：已授权" : "通知使用权：未授权（确认键去授权）");
		tvNames[1].setText(KeydroidxSettingsStorage.isNotificationBarEnabled(requireContext())
				? "桌面显示通知条：开" : "桌面显示通知条：关");
		tvNames[2].setText(KeydroidxSettingsStorage.isNotificationShowOngoing(requireContext())
				? "显示常驻通知：开" : "显示常驻通知：关");
	}
		...
		switch (focusIndex) {
			case 0:
				...
				if (!KeydroidxMusicSessionReader.openNotificationListenerSettings(requireContext())) {
					KeydroidxLog.w("NotifSettings", "当前系统版本不支持通知使用权设置页");
					Toast.makeText(requireContext(), "当前系统不支持通知使用权，请到设置→安全→通知访问手动开启",
							Toast.LENGTH_LONG).show();
				}
				return true;
			case 1:
				boolean bar = !KeydroidxSettingsStorage.isNotificationBarEnabled(requireContext());
				KeydroidxSettingsStorage.setNotificationBarEnabled(requireContext(), bar);
				refreshNames();
				return true;
			case 2:
				boolean ongoing = !KeydroidxSettingsStorage.isNotificationShowOngoing(requireContext());
				KeydroidxSettingsStorage.setNotificationShowOngoing(requireContext(), ongoing);
				// 过滤规则变了，让仓储按新规则重刷快照
