...
public class KeydroidxThemeSettingsFragment extends KeydroidxListPageFragment {

	private KeydroidxSettingsStorage storage;
	private String currentThemeId;
	private List<KeydroidxTheme.ThemeDef> themes;
	...
	public boolean onSelect() {
		if (focusIndex >= 0 && focusIndex < themes.size()) {
			KeydroidxTheme.ThemeDef chosen = themes.get(focusIndex);
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		themes = KeydroidxTheme.getThemes();
	...
	private void rebuildList() {
		...
		int rowHeight = KeydroidxDimens.dp(getResources(), 44);
		int previewSize = KeydroidxDimens.dp(getResources(), 24);
		int margin = KeydroidxDimens.dp(getResources(), 10);

		for (int i = 0; i < count; i++) {
			final KeydroidxTheme.ThemeDef item = themes.get(i);
			...
			if (item.id.equals(currentThemeId)) {
				...
				ivCheck.setImageDrawable(KeydroidxIcons.get(requireContext(), KeydroidxIcons.ICON_CHECK, item.accentColor, 18));
				row.addView(ivCheck);
			}
