 ...
 */
public class KeydroidxAdvancedSettingsFragment extends KeydroidxListPageFragment {

	// 注：原先的「通知使用权」项已移除——它与「系统与权限 → 通知中心 → 通知使用权」
	// 是同一个系统权限（enabled_notification_listeners / KeydroidxNotificationListenerService），
	// 授权入口统一收敛到通知中心设置页。
	private static final String[] ITEM_ICON_UNICODES = {
			KeydroidxIcons.ICON_TERMINAL,  // mini_shizuku
			KeydroidxIcons.ICON_POWER,     // 电源键拦截设置
	};
	...
	private KeydroidxSwitchView interceptorSwitch;
	...
	protected void onPageCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		int curMode = KeydroidxSettingsStorage.getPowerInterceptorMode(requireContext());
		KeydroidxLog.i("AdvancedSettings", "进入高级设置页，当前电源键拦截方案=" + curMode
				+ " (" + KeydroidxSettingsStorage.getPowerInterceptorModeName(curMode) + ")");
		...
		KeydroidxLog.i("AdvancedSettings", "高级设置初始化完成");
	}
	 ...
	 * 与 {@link KeydroidxPowerInterceptFragment} 的实际底层动作保持一致。
	 */
	private boolean isInterceptorOn() {
		int mode = KeydroidxSettingsStorage.getPowerInterceptorMode(requireContext());
		return mode == KeydroidxSettingsStorage.POWER_INTERCEPTOR_MODE_1
				|| mode == KeydroidxSettingsStorage.POWER_INTERCEPTOR_MODE_2;
	}
	...
	public void onResume() {
		...
		if (interceptorSwitch != null) {
			...
			KeydroidxLog.d("AdvancedSettings", "onResume 刷新电源键拦截开关: "
					+ (on ? "开" : "关") + " 方案=" + KeydroidxSettingsStorage.getPowerInterceptorModeName(
							KeydroidxSettingsStorage.getPowerInterceptorMode(requireContext())));
		}
	}

	/** 构建行内自绘开关（诺基亚风格：跑道形轨道 + 圆形滑块），仅展示状态、不响应点击。 */
	private static class KeydroidxSwitchView extends View {
		...
		KeydroidxSwitchView(Context context, boolean checked) {
			super(context);
			this.checked = checked;
			trackW = KeydroidxDimens.dp(context.getResources(), 34);
			trackH = KeydroidxDimens.dp(context.getResources(), 16);
			thumbS = trackH - KeydroidxDimens.dp(context.getResources(), 4);
			margin = KeydroidxDimens.dp(context.getResources(), 2);
			...
			public void run() {
				...
				mainHandler.post(new Runnable() {
					...
					public void run() {
						...
						KeydroidxLog.i("AdvancedSettings", "mini_shizuku 状态: " + (running ? "在线" : "离线"));
					}
				});
			}
	...
	public boolean onSelect() {
		...
		switch (focusIndex) {
			case 0:
				KeydroidxLog.i("AdvancedSettings", "进入 mini_shizuku 服务");
				((KeydroidxDesktopActivity) requireActivity()).openFragment(new ShizukuFragment());
				return true;
			case 1:
				KeydroidxLog.i("AdvancedSettings", "进入电源键拦截设置");
				((KeydroidxDesktopActivity) requireActivity()).openFragment(new KeydroidxPowerInterceptFragment());
				return true;
			default:
				return false;
		}
