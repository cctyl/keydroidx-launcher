 ...
 * {@link KeydroidxLockScreen#lock(Context)} 执行 Device Admin 锁屏。
 * 这样不依赖 {@code input keyevent}（各 ROM 兼容性差），全版本通用。
 */
public class KeydroidxLockReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		if (ACTION_LOCK_SCREEN.equals(intent.getAction())) {
			KeydroidxLog.i("LockReceiver", "收到锁屏广播，执行 Device Admin 锁屏");
