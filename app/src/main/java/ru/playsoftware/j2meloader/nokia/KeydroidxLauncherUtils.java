 ...
 */
public final class KeydroidxLauncherUtils {

	private static final String NOKIA_ACTIVITY =
			"ru.playsoftware.j2meloader.nokia.KeydroidxDesktopActivity";

	private KeydroidxLauncherUtils() {
	}
	...
	public static boolean isDefaultLauncher(Context context) {
		try {
			...
			KeydroidxLog.e("LauncherUtils", "isDefaultLauncher 判断失败", e);
			return false;
		}
	}
	 ...
	 * 若非默认桌面，显式启动 KeydroidxDesktopActivity，并附带平滑无缝过渡动画，避免生硬的 Activity 推进动画。
	 */
	public static void navigateToHome(Context context) {
		...
		if (isDefault) {
			try {
				...
				KeydroidxLog.i("LauncherUtils", "navigateToHome: 隐式 HOME 已发送 (默认桌面)");
				return;
			} catch (Exception e) {
				KeydroidxLog.w("LauncherUtils", "隐式 HOME 失败，降级显式: " + e.getMessage());
			}
		}

		// 非默认桌面或隐式失败：显式拉起 KeydroidxDesktopActivity
		...
		try {
			...
			KeydroidxLog.i("LauncherUtils", "navigateToHome: 显式 Desktop 已发送");
		} catch (Exception e) {
			KeydroidxLog.e("LauncherUtils", "navigateToHome 显式启动失败", e);
		}
	}
}
