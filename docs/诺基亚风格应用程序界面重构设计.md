...
当前原键桌面的「应用程序」界面（`KeydroidxBoxFragment`）在选中 JAR 应用后按下确认键，会进入次级菜单页面（显示"启动"/"设置"/"卸载"三个选项）。这与真实诺基亚手机的交互习惯不符：
...
```java
public class KeydroidxBoxFragment extends Fragment implements KeydroidxFocusHost {
    // 双模式
...
│  │                                                     │   │
│  │ 左软键：                                             │   │
│  │   - JAR应用 → 弹出 KeydroidxAppOptionsDialog            │   │
│  │   - 其他 → 等效确认键                               │   │
│  │                                                     │   │
│  │ 右软键 → 退出                                       │   │
│  └─────────────────────────────────────────────────────┘   │
...
#### 选项菜单弹窗（`KeydroidxAppOptionsDialog`）
...
| 卸载 | 弹出 `KeydroidxUninstallDialog` | 确认后删除应用，刷新网格 |
...
| 文件 | 类型 | 说明 |
|---|---|---|
| `KeydroidxAppOptionsDialog.java` | Java | 选项菜单弹窗（启动/设置/卸载） |
| `dialog_keydroidx_app_options.xml` | Layout | 选项菜单弹窗布局 |

#### 5.1.2 修改文件

| 文件 | 修改内容 |
|---|---|
| `KeydroidxBoxFragment.java` | 删除次级菜单模式，修改确认键和左软键行为 |

#### 5.1.3 不动文件

| 文件 | 原因 |
|---|---|
| `KeydroidxUninstallDialog.java` | 卸载弹窗复用，行为不变 |
| `dialog_keydroidx_uninstall.xml` | 卸载弹窗布局不变 |
| `KeydroidxInstallerDialog.java` | 安装弹窗独立，不受影响 |

---

### 5.2 KeydroidxBoxFragment 修改详解
...
public boolean onSoftLeft() {
    ...
    if (focusIndex >= 2) {
        ...
        if (appIdx >= 0 && appIdx < appItems.size()) {
            AppItem app = appItems.get(appIdx);
            KeydroidxLog.i("Box", "onSoftLeft: 弹出选项菜单 " + app.getTitle());
...
private void showAppOptionsMenu(AppItem app) {
    KeydroidxAppOptionsDialog dialog = KeydroidxAppOptionsDialog.newInstance(app.getTitle());
    dialog.setOptionsListener(new KeydroidxAppOptionsDialog.OptionsListener() {
        @Override
...
### 5.3 KeydroidxAppOptionsDialog 完整代码
 ...
 *
 * 按键规范：
 * - 接入 KeydroidxKeyBinding，禁止写死 keyCode
 * - 软键栏无高亮/焦点逻辑（只显示文字标签）
 * - 内容区选项使用 bg_keydroidx_selected_dark 高亮
 */
public class KeydroidxAppOptionsDialog extends DialogFragment {
    ...
    public static KeydroidxAppOptionsDialog newInstance(String appName) {
        KeydroidxAppOptionsDialog dialog = new KeydroidxAppOptionsDialog();
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        KeydroidxLog.i(TAG, "onCreateDialog: 创建选项菜单");
        Dialog dialog = new Dialog(requireActivity());
        dialog.setContentView(R.layout.dialog_keydroidx_app_options);
        ...
        dialog.setOnKeyListener((d, keyCode, event) -> {
            ...
            // 返回键单独处理（KeydroidxKeyBinding 不管 BACK）
            if (keyCode == KeyEvent.KEYCODE_BACK) {
                KeydroidxLog.i(TAG, "返回键：关闭选项菜单");
                dismiss();
                return true;
            }
    ...
    private void setFocus(int index) {
        ...
        for (int i = 0; i < OPTION_COUNT; i++) {
            if (optionRows[i] != null) {
                if (i == index) {
                    optionRows[i].setBackgroundResource(R.drawable.bg_keydroidx_selected_dark);
                } else {
...
### 5.4 布局文件 dialog_keydroidx_app_options.xml
...
### 5.5 KeydroidxBoxFragment 完整修改后代码（关键部分）
...
public class KeydroidxBoxFragment extends Fragment implements KeydroidxFocusHost {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_keydroidx_box, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (wall != null) {
            wall.setBackgroundResource(R.drawable.bg_keydroidx_box);
        }
        ...
        KeydroidxLog.i("Box", "应用程序初始化完成，等待数据加载…");
    }
...
```
用户    KeydroidxBoxFragment    KeydroidxDesktopActivity    Config    MicroActivity
 │            │                    │                  │           │
 │──方向键───►│                    │                  │           │
 │            │ 高亮"手机QQ2008"   │                  │           │
 │            │                    │                  │           │
...
```
用户    KeydroidxBoxFragment    KeydroidxAppOptionsDialog    Config
 │            │                    │                  │
 │──方向键───►│                    │                  │
 │            │ 高亮"手机QQ2008"   │                  │
 │            │                    │                  │
...
```
用户    KeydroidxBoxFragment    KeydroidxAppOptionsDialog    KeydroidxUninstallDialog    AppRepository
 │            │                    │                        │                    │
 │──左软键───►│                    │                        │                    │
 │            │ showAppOptionsMenu()│                       │                    │
 │            │──newInstance()────►│                        │                    │
...
| `KeydroidxAppOptionsDialog.java` | `app/src/main/java/ru/playsoftware/j2meloader/nokia/KeydroidxAppOptionsDialog.java`（新建） |
| `KeydroidxUninstallDialog.java` | `app/src/main/java/ru/playsoftware/j2meloader/nokia/KeydroidxUninstallDialog.java` |
| `dialog_keydroidx_app_options.xml` | `app/src/main/res/layout/dialog_keydroidx_app_options.xml`（新建） |
| `dialog_keydroidx_uninstall.xml` | `app/src/main/res/layout/dialog_keydroidx_uninstall.xml` |
| `KeydroidxKeyBinding.java` | `app/src/main/java/ru/playsoftware/j2meloader/nokia/KeydroidxKeyBinding.java` |
| `KeydroidxFocusHost.java` | `app/src/main/java/ru/playsoftware/j2meloader/nokia/KeydroidxFocusHost.java` |

### 9.2 代码规范检查清单

- [ ] `KeydroidxAppOptionsDialog` 接入 `KeydroidxKeyBinding`，禁止写死 keyCode
- [ ] 软键栏无高亮/焦点逻辑（`bg_keydroidx_selected` 不用于软键）
- [ ] 弹窗消费抬起事件（`ACTION_DOWN` 才处理）
- [ ] 返回键单独处理（`KeydroidxKeyBinding` 不管 BACK）
- [ ] 所有日志使用 `KeydroidxLog.i/d/w/e`
